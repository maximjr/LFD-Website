...
export const activateSubscription = functions.https.onCall(async (data, context) => {
  ...
  // Initial query to find the key document reference
  const keySnapshot = await db.collection("activationKeys")
  ...
  const keyDocRef = keySnapshot.docs[0].ref;

  return await db.runTransaction(async (transaction) => {
    const keyDoc = await transaction.get(keyDocRef);
    const keyData = keyDoc.data();

    if (!keyData) throw new functions.https.HttpsError('not-found', 'Invalid key');
    ...
    if (keyData.expiresAt && keyData.expiresAt.toDate() < new Date()) {
      throw new functions.https.HttpsError('failed-precondition', 'Key expired');
    }
